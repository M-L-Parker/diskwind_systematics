...
if not os.path.exists("fits_dw_3to10_err.dat"):
	outputfile1=open('fits_dw_3to10_err.dat','w')
	...
	try:
		prev_runs=np.loadtxt('fits_dw_3to10_err.dat',skiprows=1,delimiter=',',dtype='str')[:,0]
		outputfile1=open('fits_dw_3to10_err.dat','a')
	except:
		outputfile1=open('fits_dw_3to10_err.dat','w')
		titlerow="filename,a,R,Afe,mdot,fv,lx,gamma,mu,fit_mdot,fit_fv,fit_lx,fit_mu,fit_gamma,chi2,dof\n"
		outputfile1.write(titlerow)
...
for i,p in zip(samples,parsets[samples]):
	...
	if filename not in prev_runs:
		...
		Fit.error('stop 100,0.2 max 100 1. 1 2 3 4 6 7')
...
#
# outputfile2=open("fits_dw_3to10_err.dat",'w')
