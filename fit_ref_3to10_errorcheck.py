...
def read_xcm(xcmfilename):
	...
	Don't know if the Xset.restore() function existed when
	I first wrote this, it seems very redundant.
	'''
...
if not os.path.exists("fits_relxill_3to10_err.dat"):
	outputfile1=open('fits_relxill_3to10_err.dat','w')
	...
	try:
		prev_runs=np.loadtxt('fits_relxill_3to10_err.dat',skiprows=1,delimiter=',',dtype='str')[:,0]
		outputfile1=open('fits_relxill_3to10_err.dat','a')
	except:
		outputfile1=open('fits_relxill_3to10_err.dat','w')
		titlerow="filename,a,R,Afe,mdot,fv,lx,gamma,mu,fit_a,fit_h,fit_Afe,fit_gamma,fit_i,chi2,dof\n"
		outputfile1.write(titlerow)
...
samples=sorted(np.random.choice(N,size=N_samples))
...
for i,p in zip(samples,parsets[samples]):
	parset=p[1:]
	...
	if filename not in prev_runs:
		...
		Fit.error('stop 100,0.2 max 100 1. 7 10 13 14 20 22')
